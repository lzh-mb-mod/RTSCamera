    ...
    public class Patch_OrderController
    {
            ...
            bool isFromPatch = false)
        {
            ...
            try
            {
                ...
                PrepareFormationSimulationDataForWidth(formations);
                ...
                if (isLineShort)
                {
                    PrepareFormationSimuationDataForUnitSpacingOfLineShort(formations);
                ...
                {
                    PrepareFormationSimuationDataForUnitSpacingOfNonLineShort(formations);
        ...
        private static void PrepareFormationSimulationDataForWidth(IEnumerable<Formation> formations)
        {
        ...
        private static void PrepareFormationSimuationDataForUnitSpacingOfLineShort(IEnumerable<Formation> formations)
        {
        ...
        private static void PrepareFormationSimuationDataForUnitSpacingOfNonLineShort(IEnumerable<Formation> formations)
        {
            foreach (var formation in formations)
            {
            ...
            out IEnumerable<Formation> remainingFormations)
        {
            ...
            foreach (var pair in formationOrderPositionList)
            {
                ...
                bool hasEccentricPosition = GetFormationLineBeginEnd(formation, formationPosition, formationDirection, GetActualOrCurrentWidth(formation), out var begin, out var end);
                Vec2 vec = end.AsVec2 - begin.AsVec2;
                float length = vec.Length;
                vec.Normalize();
                bool flag = length.ApproximatelyEqualsTo(actualOrCurrentWidth, 0.1f);
                var formationWidth = MathF.Clamp(flag ? actualOrCurrentWidth : length, GetFormationVirtualMinimumWidth(formation), GetFormationVirtualMaximumWidth(formation));
                var eccentricFormationPosition = formationPosition;
                eccentricFormationPosition.SetVec2((begin.AsVec2 + end.AsVec2) / 2);
                ...
                DecreaseUnitSpacingAndWidthIfNotAllUnitsFit(formation, GetSimulationFormation(formation, simulationFormations), in eccentricFormationPosition, in formationDirection, ref formationWidth, ref unitSpacingReduction, actualUnitSpacing);
                SimulateNewOrderWithFrameAndWidth(formation, GetSimulationFormation(formation, simulationFormations), simulationAgentFrames, simulationFormationChanges, in eccentricFormationPosition, in formationDirection, formationWidth, unitSpacingReduction, simulateFormationDepth: true, out var simulatedFormationDepth, actualUnitSpacing);

                if (isSimulatingFormationChanges)
                {
                    LivePreviewFormationChanges.SetPreviewShape(formation, hasEccentricPosition ? eccentricFormationPosition : new WorldPosition?(), formationWidth, simulatedFormationDepth);
                }
            ...
            )
        {
            ...
            var currentStack = new StackRecord()
            {
                ...
                Width = GetActualOrCurrentWidth(formationOrderPositionList[0].Key),
            };
            for (int i = 1; i < formationOrderPositionList.Count; ++i)
            {
                ...
                if (ShouldFormationBeStackedTogether(currentStack, currentFormation, currentFormationOrderPosition, oldDragVec))
                {
                    ...
                    currentStack.Width = MathF.Max(currentStack.Width, actualOrCurrentWidth);
            ...
            out IEnumerable<Formation> remainingFormations)
        {
            ...
            dragVec.Normalize();
            var distanceBetweenFormations = 1.5f;
            ...
            for (int i = 0; i < stacksRecord.Count; ++i)
            {
                var stack = stacksRecord[i];
                var formationsInStack = stack.Formations;
                var newStackWidth = newWidths[i];
                ...
                foreach (var formation in formationsInStack)
                {
                    ...
                    var actualUnitSpacing = GetActualOrCurrentUnitSpacing(formation);
                    var actualOrCurrentWidth = GetActualOrCurrentWidth(formation);
                    var newFormationWidth = MathF.Clamp(isWidthApproximatelySame ? actualOrCurrentWidth : newStackWidth / stacksRecord[i].Width * actualOrCurrentWidth, GetFormationVirtualMinimumWidth(formation), GetFormationVirtualMaximumWidth(formation));
                    ...
                    if (isSimulatingFormationChanges)
                    {
                        ...
                        LivePreviewFormationChanges.SetPreviewShape(formation, null, newFormationWidth, simulatedFormationDepth);
                    }
                    startPoint += simulatedFormationDepth + GetGapBetweenLinesOfFormation(formation, newUnitSpacing);
                }
                offset += newStackWidth + distanceBetweenFormations;
        ...
        // Returns whether has eccentric position
        private static bool GetFormationLineBeginEnd(Formation formation, WorldPosition formationLineBegin, Vec2 direction, float formationWidth, out WorldPosition begin, out WorldPosition end)
        {
            direction.RotateCCW(-1.57079637f);
            double num2 = (double)direction.Normalize();
            var originalBegin = formationLineBegin.AsVec2 + formationWidth / 2f * direction;
            var originalEnd = formationLineBegin.AsVec2 - formationWidth / 2f * direction;
            end = Mission.Current.GetStraightPathToTarget(originalEnd, formationLineBegin);
            begin = Mission.Current.GetStraightPathToTarget(originalBegin, formationLineBegin);
            if (begin.AsVec2.DistanceSquared(end.AsVec2) < 0.001f)
            {
                var originalBeginWorldPosition = formationLineBegin;
                var originalEndWorldPosition = formationLineBegin;
                originalBeginWorldPosition.SetVec2(originalBegin);
                originalEndWorldPosition.SetVec2(originalEnd);
                end = Mission.Current.GetStraightPathToTarget(originalEnd, originalBeginWorldPosition);
                begin = Mission.Current.GetStraightPathToTarget(originalBegin, originalEndWorldPosition);
                return true;
            }
            return originalEnd.DistanceSquared(end.AsVec2) > 0.001f || originalBegin.DistanceSquared(begin.AsVec2) > 0.001f;
        }
            ...
            List<(Formation, int, float, WorldPosition, Vec2)> simulationFormationChanges)
        {
            ...
            foreach (Formation formation in formations)
            {
                ...
                if (isSimulatingFormationChanges && !isLineShort)
                {
                    var unitSpacing = MathF.Max(GetActualOrCurrentUnitSpacing(formation) - unitSpacingReduction, 0);
                    LivePreviewFormationChanges.UpdateFormationChange(formation, null, null, unitSpacing, null);
                    LivePreviewFormationChanges.SetPreviewShape(formation, null, formationWidth, simulatedFormationDepth);
                }
                ...
                foreach (var formation in formationsWithoutLocking)
                {
                    ...
                    if (LivePreviewFormationChanges.VirtualChanges.TryGetValue(formation, out var formationChange))
                    {
                        if (formationChange.MovementOrderType == OrderType.Advance)
                        {
                            ...
                            var advancePosition = GetAdvanceOrderPosition(formation, WorldPosition.WorldPositionEnforcedCache.None, targetFormation);
        ...
        public static bool Prefix_SetOrderWithPosition(OrderController __instance, OrderType orderType, WorldPosition orderPosition)
        {
            ...
            if (orderType == OrderType.LookAtDirection)
            {
                ...
                if (fadeOut)
                {
                    Patch_OrderTroopPlacer.HideOrderPositionEntities();
                }
                ...
                if (formationsWithLocking.Count > 0)
                {
                    foreach (Formation formation in formationsWithLocking)
                    {
                        if (GetActualUnitSpacings()?.TryGetValue(formation, out var actualUnitSpacing) ?? false)
                            formation.SetPositioning(unitSpacing: actualUnitSpacing);
                        if (GetActualWidths()?.TryGetValue(formation, out var customWidth) ?? false)
                    ...
                    foreach ((Formation formation, int unitSpacingReduction, float customWidth, WorldPosition position, Vec2 direction) in simulationFormationChanges)
                    {
                        ...
                        if (unitSpacingReduction > 0)
                        {
                            var oldUnitSpacing = formation.UnitSpacing;
                            var newUnitSpacing = MathF.Max(formation.UnitSpacing - unitSpacingReduction, 0);
                            formation.SetPositioning(unitSpacing: newUnitSpacing);
                            if (formation.UnitSpacing != oldUnitSpacing)
                            {
                                SetActualUnitSpacing(formation, oldUnitSpacing);
                            }
                        }
                    ...
                    if (fadeOut)
                    {
                        Patch_OrderTroopPlacer.AddOrderPositionEntities(simulationAgentFrames, true, startIndex);
            ...
            out List<(Formation, int, float, WorldPosition, Vec2)> simulationFormationChanges)
        {
            ...
            foreach (var formation in formations)
            {
                ...
                if (isSimulatingFormationChanges)
                {
                    LivePreviewFormationChanges.SetPreviewShape(formation, null, width, simulatedFormationDepth);
                }
            }
        }
            ...
            out List<(Formation, int, float, WorldPosition, Vec2)> simulationFormationChanges)
        {
            ...
            foreach (var pair in formationOrderPositionDictionary)
            {
                ...
                bool hasEccentricPosition =  GetFormationLineBeginEnd(formation, formationPosition, newDirection, actualOrCurrentWidth, out var begin, out var end);
                Vec2 vec = end.AsVec2 - begin.AsVec2;
                float length = vec.Length;
                vec.Normalize();
                bool flag = length.ApproximatelyEqualsTo(actualOrCurrentWidth, 0.1f);
                ...
                int unitSpacingReduction = 0;
                var actualUnitSpacing = GetActualOrCurrentUnitSpacing(formation);
                DecreaseUnitSpacingAndWidthIfNotAllUnitsFit(formation, GetSimulationFormation(formation, simulationFormations), in eccentricFormationPosition, in newDirection, ref formationWidth, ref unitSpacingReduction, actualUnitSpacing);
                SimulateNewOrderWithFrameAndWidth(formation, GetSimulationFormation(formation, simulationFormations), simulationAgentFrames, simulationFormationChanges, in eccentricFormationPosition, in newDirection, formationWidth, unitSpacingReduction, true, out var simulatedFormationDepth, actualUnitSpacing);
                if (isSimulatingFormationChanges)
                {
                    LivePreviewFormationChanges.SetPreviewShape(formation, hasEccentricPosition ? eccentricFormationPosition : new WorldPosition?(), formationWidth, simulatedFormationDepth);
                }
        ...
        {
            bool hasFormation = LivePreviewFormationChanges.VirtualChanges.ContainsKey(formation);
            return hasFormation && LivePreviewFormationChanges.VirtualChanges[formation].WorldPosition != null ? LivePreviewFormationChanges.VirtualChanges[formation].WorldPosition.Value : formation.OrderPosition.IsValid ? formation.CreateNewOrderWorldPosition(WorldPosition.WorldPositionEnforcedCache.None) : GetFormationCurrentPositionAsWorldPosition(formation);
        }

        public static WorldPosition GetFormationVirtualPositionWithEccentricPosition(Formation formation)
        {
            bool hasFormation = LivePreviewFormationChanges.VirtualChanges.ContainsKey(formation);
            return hasFormation && LivePreviewFormationChanges.VirtualChanges[formation].EccentricPosition != null ? LivePreviewFormationChanges.VirtualChanges[formation].EccentricPosition.Value : GetFormationVirtualPosition(formation);
        }
        ...
        private static void TryIntializeFormationChanges(Formation formation)
        {
            ...
            {
                LivePreviewFormationChanges.UpdateFormationChange(formation, formation.CreateNewOrderWorldPosition(WorldPosition.WorldPositionEnforcedCache.None), null, null, null);
            }
            ...
            out List<(Formation, int, float, WorldPosition, Vec2)> simulationFormationChanges)
        {
            ...
            foreach (var formation in formations)
            {
                ...
                DecreaseUnitSpacingAndWidthWithNewArrangementOrderIfNotAllUnitsFit(formation, GetSimulationFormation(formation, simulationFormations), oldArrangementOrder, newArrangementOrder, in formationPosition, in direction, width, out var newWidth, unitSpacing, maxNewUnitSpacing, out var unitSpacingReduction);
                SimulateNewArrangementOrderWithFrameAndWidth(formation, GetSimulationFormation(formation, simulationFormations), oldArrangementOrder, newArrangementOrder, simulationAgentFrames, simulationFormationChanges, in formationPosition, in direction, width, newWidth, unitSpacing, maxNewUnitSpacing, unitSpacingReduction, true, out var simulatedFormationDepth);
                if (isSimulatingFormationChanges)
                {
                    LivePreviewFormationChanges.SetArrangementOrder(newArrangementOrder, new List<Formation> { formation });
                    LivePreviewFormationChanges.UpdateFormationChange(formation, null, null, maxNewUnitSpacing - unitSpacingReduction, newWidth);
                    LivePreviewFormationChanges.SetPreviewShape(formation, null, newWidth, simulatedFormationDepth);
                }
            }
        }
            ...
            int newUnitSpacing)
        {
            ...
            var unitSpacing = GetFormationVirtualUnitSpacing(formation) ?? formation.UnitSpacing;
            if (oldArrangementOrder != ArrangementOrder.ArrangementOrderEnum.Column && newArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Column)
            {
                return MathF.Clamp(oldWidth * 0.1f, Utilities.Utility.GetFormationMinimumWidthOfArrangementOrder(formation, newArrangementOrder, unitSpacing), Utilities.Utility.GetFormationMaximumWidthOfArrangementOrder(formation, newArrangementOrder));
            }
            if (oldArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Column && newArrangementOrder != ArrangementOrder.ArrangementOrderEnum.Column)
            {
                return MathF.Clamp(oldWidth / 0.1f, Utilities.Utility.GetFormationMinimumWidthOfArrangementOrder(formation, newArrangementOrder, unitSpacing), Utilities.Utility.GetFormationMaximumWidthOfArrangementOrder(formation, newArrangementOrder));
            ...
            if (oldArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Circle)
            {
                oldFlankWidth = Utilities.Utility.ConvertFromWidthToFlankWidthOfCircularFormation(formation, oldUnitSpacing, oldWidth);
            }
            else if (oldArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Square)
            {
                oldFlankWidth = Utilities.Utility.ConvertFromWidthToFlankWidthOfSquareFormation(formation, oldUnitSpacing, oldWidth);
            }
            ...
            if (newArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Circle)
            {
                newWidth = Utilities.Utility.ConvertFromFlankWidthToWidthOfCircularFormation(formation, newUnitSpacing, newFlankWidth);
            }
            else if (newArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Square)
            {
                if (CommandSystemConfig.Get().HollowSquare)
            ...
            newWidth = MathF.Clamp(newWidth, Utilities.Utility.GetFormationMinimumWidthOfArrangementOrder(formation, newArrangementOrder, newUnitSpacing), Utilities.Utility.GetFormationMaximumWidthOfArrangementOrder(formation, newArrangementOrder));
            return newWidth;
        }
        ...
        private static void DecreaseUnitSpacingAndWidthWithNewArrangementOrderIfNotAllUnitsFit(Formation formation,
            ...
            in float formationWidth,
            out float newFormationWidth,
            in int unitSpacing,
            in int maxNewUnitSpacing,
            out int unitSpacingReduction)
        {
            ...
            if (unitIndex >= 0)
            {
                if (CommandSystemConfig.Get().CircleFormationUnitSpacingPreference <= CircleFormationUnitSpacingPreference.Tight && arrangementOrder == ArrangementOrder.ArrangementOrderEnum.Circle)
                {
                    ...
                    do
                    {
                        newFormationWidth = GetNewWidthOfArrangementOrder(formation, oldArrangementOrder, arrangementOrder, oldWidth, unitSpacing, maxNewUnitSpacing - unitSpacingReduction);
                        GetUnitPositionWithIndexAccordingToNewOrder(formation, simulationFormation, oldArrangementOrder, arrangementOrder, unitIndex, in formationPosition, in formationDirection, formation.Arrangement, formationWidth, newFormationWidth, unitSpacing, maxNewUnitSpacing - unitSpacingReduction, formation.Arrangement.UnitCount, formation.HasAnyMountedUnit, formation.Index, out var unitSpawnPosition, out var _, out actualWidth);
                        if (unitSpawnPosition.HasValue)
                        {
                            break;
                        }
                    ...
                    // if the above loop has recycled at least twice, which means that the current newFormationWidth is not small enough.
                    if (maxNewUnitSpacing - unitSpacingReduction > 0)
                    {
                        newFormationWidth = actualWidth;
                    }
                }
                else
                {
                    ...
                    // if the above loop has recycled at least twice, which means that the current newFormationWidth is not large enough.
                    // Commented the below condition because:
                    // For square formation, the newFormationWidth may be larger than needed, because square formation requires (fileCount mod 4) = 0, but the fileCount directly derived from newFormaitonWidth may be a little larger.
                    // So we need to just set newFormationWidth to the actualWidth.
                    //if (unitSpacingReduction > 0)
                    {
                        newFormationWidth = actualWidth;
                    }
        ...
        private static void SimulateNewArrangementOrderWithFrameAndWidth(Formation formation,
            ...
            float newFormationWidth,
            in int unitSpacing,
            in int maxNewUnitSpacing,
            in int unitSpacingReduction,
        ...
        private static void GetUnitPositionWithIndexAccordingToNewOrder(
            ...
            float width,
            float? expectedNewWidth,
            int unitSpacing,
            int? expectedNewUnitSpacing,
            int unitCount,
            bool isMounted,
            int index,
            ...
            out float newWidth)
        {
            ...
            bool isUnitSpacingTheSame = simulationFormation.UnitSpacing == unitSpacing;
            bool isUnitSpacingTheSameWithNewArrangement = expectedNewUnitSpacing != null && simulationFormation.UnitSpacing == expectedNewUnitSpacing.Value;
            if ((isUnitSpacingTheSame || isUnitSpacingTheSameWithNewArrangement) &&
                //((double)MathF.Abs((float)((double)simulationFormation.Width - (double)width + 9.9999997473787516E-06)) < (double)simulationFormation.Interval + (double)simulationFormation.UnitDiameter - 9.9999997473787516E-06 || (double)width < (double)simulationFormation.MinimumWidth && (double)MathF.Abs(simulationFormation.Width - simulationFormation.MinimumWidth) < 9.9999997473787516E-06) && 
                simulationFormation.OrderPositionIsValid)
            {
                Vec3 orderGroundPosition = simulationFormation.OrderGroundPosition;
                worldPosition = formationPosition;
                vec3 = worldPosition.GetGroundVec3();
                if (orderGroundPosition.NearlyEquals(in vec3, 0.1f))
                {
        ...
        label_2:
            ...
            {
                // FormOrder.CustomFlankWidth is actually the formation width. It's not flank with.
                _unitSpacing.SetValue(simulationFormation, expectedNewUnitSpacing.Value);
                _formOrder.SetValue(simulationFormation, FormOrder.FormOrderCustom(expectedNewWidth.Value));
                UpdateDesiredFileCountFromWidth(simulationFormation, newArrangementOrder.Value, expectedNewWidth.Value, expectedNewUnitSpacing.Value);
                simulationFormation.SetArrangementOrder(Utilities.Utility.GetArrangementOrder(newArrangementOrder.Value));
                simulationFormation.SetPositioning(unitSpacing: new int?(expectedNewUnitSpacing.Value));
                simulationFormation.SetFormOrder(FormOrder.FormOrderCustom(expectedNewWidth.Value));
            }
        label_3:
        ...
        private static void UpdateDesiredFileCountFromWidth(Formation formation, ArrangementOrder.ArrangementOrderEnum newArrangementOrder, float width, int unitSpacing)
        {
            ...
            if (newArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Circle)
            {
                flankWidth = Utilities.Utility.ConvertFromWidthToFlankWidthOfCircularFormation(formation, unitSpacing, width);
            }
            else if (newArrangementOrder == ArrangementOrder.ArrangementOrderEnum.Square)
            {
                flankWidth = Utilities.Utility.ConvertFromWidthToFlankWidthOfSquareFormation(formation, unitSpacing, width);
            }
            var fileCount = Utilities.Utility.GetUnlimitedFileCountFromWidth(formation, flankWidth, unitSpacing);
            AccessTools.Field(typeof(Formation), "_desiredFileCount").SetValue(formation, fileCount);
        }
        ...
        private static float GetFormationVirtualMinimumWidth(Formation formation)
        {
            var arrangementOrder = GetFormationVirtualArrangementOrder(formation);
            var unitSpacing = GetFormationVirtualUnitSpacing(formation) ?? formation.UnitSpacing;
            return Utilities.Utility.GetFormationMinimumWidthOfArrangementOrder(formation, arrangementOrder, unitSpacing);
        }

        private static float GetFormationVirtualMaximumWidth(Formation formation)
        {
            var arrangementOrder = GetFormationVirtualArrangementOrder(formation);
            return Utilities.Utility.GetFormationMaximumWidthOfArrangementOrder(formation, arrangementOrder);
        }
        ...
        public static void TryFadeOutForFacingToEnemyOrder(OrderController orderController, IEnumerable<Formation> selectedFormations, Formation targetFormation, bool updateFormationChanges)
        {
            bool fadeOut = Utilities.Utility.ShouldFadeOut();
            SimulateFacingToEnemyOrder(selectedFormations,
                orderController.simulationFormations,
                ...
                fadeOut,
                out var simulationAgentFrames,
                updateFormationChanges, // we may need to pass true to update formation changes here.
                out _);
            if (fadeOut)
            {
                Patch_OrderTroopPlacer.HideOrderPositionEntities();
