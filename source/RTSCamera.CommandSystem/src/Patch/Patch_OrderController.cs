...
namespace RTSCamera.CommandSystem.Patch
{
    public class Patch_OrderController
    {
            ...
            bool isFormationLayoutVertical = true)
        {
            try
            {
                if (!ShouldOverrideOrderController(formations))
                    return true;
            ...
            ref List<WorldPosition> simulationAgentFrames)
        {
            ...
            if (!Utilities.Utility.ShouldLockFormationDuringLookAtDirection(formations))
            {
                foreach (var formation in formations)
                {
                    if (Utilities.Utility.IsFormationOrderPositionMoving(formation))
                    ...
                    if (LivePreviewFormationChanges.VirtualChanges.TryGetValue(formation, out var formationChange))
                    {
                        if (formationChange.MovementOrderType == OrderType.Advance)
                        {
                            ...
                            var advanceDirection = GetAdvanceOrFallbackOrderDirection(formation, targetFormation);
                            var advancePosition = GetAdvanceOrderPosition(formation, WorldPosition.WorldPositionEnforcedCache.None, targetFormation);

                            LivePreviewFormationChanges.UpdateFormationChange(formation, advancePosition, advanceDirection, null, null);
                        }
                    }
                }
                SimulateNewFacingOrderWithoutLockingFormations(formations,
            ...
            {
                SimulateNewFacingOrderWithLockingFormations(formations,
                    simulationFormations,
                    direction,
                    true,
                    out simulationAgentFrames,
        ...
        public static bool Prefix_SetOrderWithPosition(OrderController __instance, OrderType orderType, WorldPosition orderPosition)
        {
            ...
            {
                if (Utilities.Utility.ShouldLockFormationDuringLookAtDirection(__instance.SelectedFormations))
                {
                    SimulateNewFacingOrderWithLockingFormations(__instance.SelectedFormations,
        ...
        public static WorldPosition GetFormationVirtualAveragePosition(Formation formation)
        {
        ...
        public static Vec2 GetAdvanceOrFallbackOrderDirection(Formation f, Formation targetFormation)
        {
            var enemyQuerySystem = GetTargetOrClosestEnemyFormationQuerySystem(f, targetFormation);
            return enemyQuerySystem == null ? Vec2.One : (enemyQuerySystem.MedianPosition.AsVec2 - GetFormationVirtualAveragePositionVec2(f)).Normalized();
        }
        ...
        public static WorldPosition GetAdvanceOrderPosition(Formation f, WorldPosition.WorldPositionEnforcedCache worldPositionEnforcedCache, Formation targetFormation)
        {
            ...
            var enemyQuerySystem = f.TargetFormation?.QuerySystem ?? commandQuerySystem.ClosestEnemyFormation?.QuerySystem;
            WorldPosition targetPosition;
            if (enemyQuerySystem == null)
            {
                Agent closestEnemyAgent = commandQuerySystem.ClosestEnemyAgent;
            ...
            {
                Vec2 directionAux = GetAdvanceOrFallbackOrderDirection(f, targetFormation);
                targetPosition.SetVec2(targetPosition.AsVec2 - directionAux * querySystem.MissileRangeAdjusted);
            }
            else if (enemyQuerySystem != null)
            {
                Vec2 vec2 = (enemyQuerySystem.AveragePosition - GetFormationVirtualAveragePositionVec2(f)).Normalized();
                float num = 2f;
        ...
        {
            Vec2 direction = GetAdvanceOrFallbackOrderDirection(f, targetFormation);
            WorldPosition averagePosition = GetFormationVirtualAveragePosition(f);
            averagePosition.SetVec2(f.QuerySystem.AveragePosition - direction * 7f);
        ...
        {
            if (Utilities.Utility.ShouldLockFormationDuringLookAtDirection(orderController.SelectedFormations))
            {
                order.ShouldLockFormationInFacingOrder = true;
                SimulateNewFacingOrderWithLockingFormations(orderController.SelectedFormations,
                    orderController.simulationFormations,
                    OrderController.GetOrderLookAtDirection(orderController.SelectedFormations, missionScreen.GetOrderFlagPosition().AsVec2),
                    false,
                    out _,
                    true,
                    out var simulationFormationChanges);
            ...
            {
                order.ShouldLockFormationInFacingOrder = false;
                order.PositionBegin = new WorldPosition(Mission.Current.Scene, UIntPtr.Zero, missionScreen.GetOrderFlagPosition(), false);
                SimulateNewFacingOrderWithoutLockingFormations(orderController.SelectedFormations,
                    orderController.simulationFormations,
                    OrderController.GetOrderLookAtDirection(orderController.SelectedFormations, missionScreen.GetOrderFlagPosition().AsVec2),
                    false,
                    out _,
        ...
        private static bool ShouldOverrideOrderController(IEnumerable<Formation> selectedFormations)
        {
            var team = selectedFormations.FirstOrDefault()?.Team;
            return team != null && team == Mission.Current.PlayerTeam && (team.IsPlayerGeneral || team.IsPlayerSergeant && selectedFormations.All(f => f.PlayerOwner == Agent.Main));
        }
            ...
            out List<(Formation, int, float, WorldPosition, Vec2)> simulationFormationChanges)
        {
            ...
            foreach (var formation in formations)
            {
                ...
                SimulateNewArrangementOrderWithFrameAndWidth(formation, GetSimulationFormation(formation, simulationFormations), oldArrangementOrder, newArrangementOrder, simulationAgentFrames, simulationFormationChanges, in formationPosition, in direction, width, unitSpacingReduction, false, out var _, actualUnitSpacing);
                if (isSimulatingFormationChanges)
                {
            ...
            ArrangementOrder.ArrangementOrderEnum newArrangementOrder)
        {
            ...
            var newFlankWidth = oldFlankWidth;
            ...
            else if (Utilities.Utility.GetTypeOfArrangement(oldArrangementOrder) == Utilities.Utility.GetTypeOfArrangement(ArrangementOrder.ArrangementOrderEnum.Line))
            {
            ...
            else if (Utilities.Utility.GetTypeOfArrangement(newArrangementOrder) == Utilities.Utility.GetTypeOfArrangement(ArrangementOrder.ArrangementOrderEnum.Line))
            {
            ...
            out float newWidth)
        {
            ...
            Vec2 direction;
            var oldArrangementType = Utilities.Utility.GetTypeOfArrangement(oldArrangementOrder);
            if (simulationFormation.UnitSpacing == unitSpacing && /*(double)MathF.Abs((float)((double)simulationFormation.Width - (double)width + 9.9999997473787516E-06)) < (double)simulationFormation.Interval + (double)simulationFormation.UnitDiameter - 9.9999997473787516E-06 &&*/ simulationFormation.OrderPositionIsValid && simulationFormation.OrderGroundPosition.NearlyEquals(formationPosition.GetGroundVec3(), 0.1f))
            {
                ...
                var newArrangementType = newArrangementOrder == null ? null : Utilities.Utility.GetTypeOfArrangement(newArrangementOrder.Value);
                var simulationFormationArrangementType = simulationFormation.Arrangement.GetType();
                var simulationFormationArrangementOrderType = Utilities.Utility.GetTypeOfArrangement(simulationFormation.ArrangementOrder.OrderEnum);
                if (direction.NearlyEquals(formationDirection, 0.1f))
                {
                    if (simulationFormationArrangementType == arrangement.GetType())
                        goto label_1;
                    else if (direction.NearlyEquals(formationDirection, 0.1f) && simulationFormationArrangementType == oldArrangementType && simulationFormationArrangementType == oldArrangementType)
                        goto label_2;
