...
这是一个骑马与砍杀2：霸主的mod，让玩家能在战斗中以自由视角指挥，并提供更好的指挥系统。
...
- 指挥系统 Command System
  ...
  - 新增齐射指令。
    ...
    - 自动齐射：士兵会瞄准并等待其他士兵，当大多数士兵准备好以后开火。
  ...
  - 修复了游戏本体中跟编队宽度和单位间距相关的问题。

  - 修复了游戏本体中远程部队的开战指令可能导致游戏卡死的问题。

  - 可通过鼠标中键点击敌方单位来下达集中冲锋指令。
...
- 指挥系统 Command System
  ...
  - 鼠标中键点击敌人士兵来下达集中冲锋指令。
    ...
    - 鼠标中键点击同时按住`Alt`键，可在下达集中攻击指令的同时维持原有的移动指令不变。例如，**让你的弓箭手待在原地同时攻击指定的目标**。

  - 鼠标中键点击你的士兵来选择你的编队。

  - 可通过点击兵牌来选择部队，或点击命令按钮来发出命令。
   
    - 可以按`Alt`显示鼠标。

  - 自动齐射：按`F3 F6`（传统命令布局下`F8 F1`）或`H`。

  - 手动齐射：按`F3 F7`（传统命令布局下`F8 F2`）或`J`。

  - 齐射开火：按`F3 F8`（传统命令布局下`F8 F3`）或`K`。
...
### 指挥系统Command System
  - 按住`Shift`下达指令时，可将指令添加到队列。你的部队会依次执行队列中的指令。
...
- 鼠标中键点击同时按住`Alt`键，可在下达集中攻击指令的同时维持原有的移动指令不变。

  - 对于弓箭手，可以让他们攻击指定目标同时待在原地。

  - 注意对于近战士兵，他们仍会执行之前的移动指令，这可能让他们无法攻击到目标。

- 下达面朝敌方指令的同时将鼠标移至敌方编队图标上，可下达面朝该敌方编队的指令。
...
- 新的指令“防御保持”让在防御阵型（盾墙，方阵，圆阵）下的你的士兵保持原位并举盾。
...
## 战帆DLC 相关
### RTS Camera
- 切换到自由视角模式时，AI 会控制你的角色并操纵玩家的船只。你可以向玩家船只下达船只命令，例如“开战”、“游击”等。

- 当玩家船由 AI 控制，选所有编队时将不再把玩家船只排除在外，这样更方便下达全军进攻命令。

- 当玩家受伤并切换到另一艘船上的士兵时，船只命令会相应更新，以便你可以对原来的船只下达船只命令（开战，游击等），对新的船只下达部队命令（防御船只，冲锋）等。

- 当你控制玩家角色但未开船时，会新增让士兵驾驶你的船只的命令（快捷键 F5）。该功能与舵手（Helmsman） mod 相同。

  为避免冲突与异常行为，当 RTS Camera 检测到已安装舵手（Helmsman）时，它不会为第一艘船添加该新命令。

  对于其他船只，舵手（Helmsman）不生效，因此 RTS Camera 会为这些船只添加该新命令。

  - 注意：在舵手（Helmsman） v1.0.0 中，当你正在操纵其他船只时使用舵手（Helmsman）切换第一艘船的控制方式，可能会导致你的玩家角色停止操纵船只。
